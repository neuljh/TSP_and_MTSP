...
# solving a problem: N travlers start at K random city
# finish? no 输入的序列是一整个序列 无法得到分割信息


class MTSPRandomCity(Data):
    def __init__(self, city_positions, num_travelers, model):
    ...
    def run(self):
        ...
        return self.model.g_best.target.fitness, \
               MTSPRandomCity.from_breakpoints_to_routes_without_startpoints(self.problem.get_breakpoints(), self.problem.decode_solution(self.model.g_best.solution)["per_var"])

    @staticmethod
    def from_breakpoints_to_routes_without_startpoints(breakpoints, routes):
        route = [[] for _ in range(len(breakpoints))]
        output_route = []
        sum = 0
        ...
        for i in range(0, len(output_route) - 1):
            route[i] = routes[output_route[i]:output_route[i + 1]]
        return route

    @staticmethod
    def get_breakpoints(routes, num_travlers):
        # 初始化子集列表
        subsets = [[] for _ in range(num_travlers)]
        for route in routes:
            r = random.randint(1, num_travlers)
            subsets[r - 1].append(route)
        breakpoints = []
        for subset in subsets:
            breakpoints.append(len(subset))
        return breakpoints
...
class MTspProblem(Problem):
    def __init__(self, bounds=None, minmax="min", data=None, **kwargs):
        self.data = data
        self.breakpoints = MTSPRandomCity.get_breakpoints(list(range(0, self.data["num_cities"])), self.data["num_travelers"])
    ...
    def obj_func(self, x):
        ...
        routes = MTSPRandomCity.from_breakpoints_to_routes_without_startpoints(self.breakpoints, x["per_var"])
...
models = models(epoch=10000, pop_size=30)
...
for i in range(0, len(models.get_models())):
    p = MTSPRandomCity(city_positions, num_travelers, models.get_models()[i])
    score, res = p.run()
    print(res)
...
# # 使用带有约束条件的MTSP问题
# mtsp_problem_with_constraints = MTspProblem(bounds=bounds,
...
# data = MTSPRandomCity.from_breakpoints_to_routes(mtsp_problem_with_constraints.get_breakpoints(), mtsp_problem_with_constraints.decode_solution(model_mtsp.g_best.solution)["per_var"])
# print(f"best routes: {data}")
